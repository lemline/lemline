...
internal class WaitOutboxTest {
    ...
    fun setupTest() {
        ...
        // Create a new outbox initialPosition with the mocks
        outbox = WaitOutbox(
    ...
    fun `WaitOutbox should process pending messages and mark them as sent`() {
        ...
        val updatedMessage = entityManager.find(WaitModel::class.java, message.id)
    ...
    fun `cleanupOutbox should delete old sent messages`() {
        // Given
        val oldMessage = WaitModel().apply {
            message = "old message"
            status = OutBoxStatus.SENT
            delayedUntil = Instant.now().minus(8, ChronoUnit.DAYS)
        }
        val recentMessage = WaitModel().apply {
    ...
    fun `cleanupOutbox should not delete non-sent messages`() {
        // Given
        val oldPendingMessage = WaitModel().apply {
            message = "old pending message"
            status = OutBoxStatus.PENDING
            delayedUntil = Instant.now().minus(8, ChronoUnit.DAYS)
        }
        val oldFailedMessage = WaitModel().apply {
