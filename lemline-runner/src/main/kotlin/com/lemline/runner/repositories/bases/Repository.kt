...
abstract class Repository<T> {
     ...
     */
    internal abstract val prepareStatementMap: Map<String, (PreparedStatement, T, Int) -> Unit>
    ...
    internal fun ResultSet.toModels(): List<T> = buildList {
        while (next()) {
    ...
    val getIDV7: (ResultSet, String) -> IDV7? by lazy {
        when (databaseManager.dbType) {
     ...
     */
    internal suspend fun <R> withConnection(connection: Connection?, block: suspend (Connection) -> R): R =
        withContext(Dispatchers.IO) {
            when (connection) {
...
@ExperimentalTime
internal fun ResultSet.getInstant(col: String): Instant? = getTimestamp(col)?.toInstant()?.toKotlinInstant()
