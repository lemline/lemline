...
class ExpressionEvaluatorTest {
    ...
    private val emptyScope = "{}".toJsonElement() as JsonObject

    @Test
    fun `should handle simple string interpolation with property access`() {
        val input = """{"name": "World"}""".toJsonElement()
        val filter = "\"Hello, \" + .name + \"!\""

        val result = JQExpression.eval(input, filter, emptyScope)
    ...
    fun `should handle nested property access in string interpolation`() {
        val input = """{"user": {"name": "Alice", "age": 30}}""".toJsonElement()
    ...
    fun `should handle multiple interpolations in echo command using concatenation`() {
        val input = """{"greeting": "Hello", "target": "World", "punctuation": "!"}""".toJsonElement()
    ...
    fun `should handle array access in string interpolation`() {
        val input = """{"items": ["first", "second", "third"]}""".toJsonElement()
    ...
    fun `should handle complex shell command with interpolation`() {
        val input = """{"filename": "data.txt", "directory": "/tmp", "pattern": "error"}""".toJsonElement()
    ...
    fun `should handle string interpolation with filters and functions`() {
        val input = """{"name": "john doe", "count": 5}""".toJsonElement()
    ...
    fun `should handle string interpolation with conditional filters`() {
        val input = """{"status": "active", "name": "service"}""".toJsonElement()
        val filter =
    ...
    fun `should handle null values in interpolation`() {
        val input = """{"existing": "value", "null_field": null}""".toJsonElement()
    ...
    fun `should handle special characters in interpolated strings`() {
        val input =
            """{"message": "Hello \"quoted\" string with 'single quotes'", "path": "/path/with spaces/file.txt"}""".toJsonElement()
    ...
    fun `should handle mathematical operations in interpolation`() {
        val input = """{"a": 10, "b": 5}""".toJsonElement()
    ...
    fun `should handle string concatenation using plus operator`() {
        val input = """{"first": "Hello", "second": "World"}""".toJsonElement()
    ...
    fun `should handle object property access with dynamic keys`() {
        val input =
            """{"config": {"production": "prod-value", "development": "dev-value"}, "environment": "production"}""".toJsonElement()
    ...
    fun `should handle array filtering and simple access`() {
        val input =
            """{"users": [{"name": "Alice", "active": true}, {"name": "Bob", "active": false}, {"name": "Charlie", "active": true}]}""".toJsonElement()
    ...
    fun `should handle nested interpolations with scope variables`() {
        val input = """{"name": "Alice"}""".toJsonElement()
        val scope = """{"prefix": "Dr."}""".toJsonElement() as JsonObject
        val filter = "\"Greeting: Hello, \" + \$prefix + \" \" + .name + \"!\""

        val result = JQExpression.eval(input, filter, scope)
    ...
    fun `should handle environment variable style interpolation`() {
        val input = """{"HOME": "/home/user", "USER": "alice", "PATH": "/usr/bin:/bin"}""".toJsonElement()
    ...
    fun `should handle interpolation with type conversion`() {
        val input = """{"number": 42, "boolean": true, "array": [1, 2, 3]}""".toJsonElement()
    ...
    fun `should handle error cases gracefully with invalid interpolation`() {
        val input = """{"valid": "value"}""".toJsonElement()

        try {
            val filter = "\"Invalid: \\(.valid | invalid_function)\""
            JQExpression.eval(input, filter, emptyScope)
    ...
    fun `should handle multiple nested object access`() {
        val input =
            """{"app": {"database": {"connection": {"host": "localhost", "port": 5432, "database": "myapp"}}}}""".toJsonElement()
        val filter =
    ...
    fun `should handle string interpolation in complex command with escaping`() {
        val input = """{"search_term": "error message", "log_file": "/var/log/app.log"}""".toJsonElement()
    ...
    fun `should handle complex shell command construction`() {
        val input = """{"source_dir": "/source", "target_dir": "/target", "file_pattern": "*.log"}""".toJsonElement()
    ...
    fun `should handle conditional string building`() {
        val input = """{"debug": true, "command": "ls"}""".toJsonElement()
    ...
    fun `should handle array access operations for command arguments`() {
        val input = """{"args": ["-l", "-a", "--color=auto"]}""".toJsonElement()
    ...
    fun `should handle URL construction with interpolation`() {
        val input =
            """{"protocol": "https", "host": "api.example.com", "port": 443, "path": "/v1/users", "id": 123}""".toJsonElement()
    ...
    fun `should handle JSON-like string construction`() {
        val input = """{"name": "test", "value": 42, "enabled": true}""".toJsonElement()
