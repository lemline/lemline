...
data class NodePosition(
     ...
     */
    val jsonPointer =
        if (path.isEmpty()) com.lemline.swruntime.sw.tasks.JsonPointer.root else com.lemline.swruntime.sw.tasks.JsonPointer(
            "/${path.joinToString("/")}"
        )
    ...
    fun addName(name: String): NodePosition {
        ...
        com.lemline.swruntime.sw.tasks.Token.entries.map { it.token }.let {
     ...
     */
    fun addToken(token: com.lemline.swruntime.sw.tasks.Token): NodePosition =
    ...
    companion object {
        val root = com.lemline.swruntime.sw.tasks.JsonPointer.root.toPosition()
    }
}