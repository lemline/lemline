...
data class NodePosition(
     ...
     */
    val jsonPointer =
        if (path.isEmpty()) com.lemline.runtime.sw.tasks.JsonPointer.root else com.lemline.runtime.sw.tasks.JsonPointer(
            "/${path.joinToString("/")}"
        )
    ...
    fun addName(name: String): NodePosition {
        ...
        com.lemline.runtime.sw.tasks.Token.entries.map { it.token }.let {
     ...
     */
    fun addToken(token: com.lemline.runtime.sw.tasks.Token): NodePosition =
    ...
    companion object {
        val root = com.lemline.runtime.sw.tasks.JsonPointer.root.toPosition()
    }
}