...
abstract class JacksonSerializer<T : Any>(private val targetClassJava: Class<T>) : KSerializer<T> {
    ...
    protected open val jacksonMapper: ObjectMapper by lazy { com.lemline.common.json.Json.jacksonMapper }
    protected open val kotlinxJson: Json by lazy { com.lemline.common.json.Json.json } // Use the configured instance

    // Use the descriptor for JsonElement from the configured instance
    override val descriptor: SerialDescriptor by lazy { kotlinxJson.serializersModule.serializer<JsonElement>().descriptor }
    ...
    override fun deserialize(decoder: Decoder): T {
        ...
        val jsonString =
            kotlinxJson.encodeToString(kotlinxJson.serializersModule.serializer<JsonElement>(), jsonElement)
