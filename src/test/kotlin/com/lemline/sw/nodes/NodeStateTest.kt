    ...
    inner class SerializationTests {
        ...
        fun setup() {
            state = NodeState().apply {
                variables = Json.jsonObject.set("testVar", "value")
                attemptIndex = 1
                childIndex = 2
                rawInput = Json.jsonObject.set("input", "test")
        ...
        inner class ErrorHandlingTests {
            ...
            fun `test invalid variables type`() {
                val json = Json.jsonObject.set(NodeState.VARIABLES, "invalid")
                assertThrows(IllegalArgumentException::class.java) {
                    Json.decodeFromElement<NodeState>(json)
                }
            }

            @Test
            fun `test invalid attempt index type`() {
                val json = Json.jsonObject.set(NodeState.ATTEMPT_INDEX, "invalid")
                assertThrows(IllegalArgumentException::class.java) {
                    Json.decodeFromElement<NodeState>(json)
                }
            }
            ...
            fun `test invalid child index type`() {
                val json = Json.jsonObject.set(NodeState.CHILD_INDEX, "invalid")
                assertThrows(IllegalArgumentException::class.java) {
                    Json.decodeFromElement<NodeState>(json)
                }
            }
            ...
            fun `test invalid context type`() {
                val json = Json.jsonObject.set(NodeState.CONTEXT, "invalid")
                assertThrows(IllegalArgumentException::class.java) {
                    Json.decodeFromElement<NodeState>(json)
                }
            }
            ...
            fun `test invalid started at format`() {
                val json = Json.jsonObject.set(NodeState.STARTED_AT, "invalid-date")
            ...
            fun `test invalid for index type`() {
                val json = Json.jsonObject.set(NodeState.FOR_INDEX, "invalid")
                assertThrows(IllegalArgumentException::class.java) {
                    Json.decodeFromElement<NodeState>(json)
                }
            }
