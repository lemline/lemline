...
class ForkRepository : WithInstanceRepository<ForkModel>() {
        ...
        if (branches.isNotEmpty()) {
            conn.prepareStatement(
                ...
                branches.forEach { branch ->
                    ...
                    stmt.setTimestamp(8, branch.startedAt?.toJavaInstant()?.let { Timestamp.from(it) })
                    stmt.setTimestamp(9, branch.completedAt?.toJavaInstant()?.let { Timestamp.from(it) })
                    stmt.setTimestamp(10, Timestamp.from(branch.createdAt.toJavaInstant()))
                    stmt.setTimestamp(11, Timestamp.from(branch.updatedAt.toJavaInstant()))
                    stmt.addBatch()
                }
                stmt.executeBatch()
            }
    ...
    ): ForkModel? = withConnection(null) { conn ->
    ...
    suspend fun recordBranchCompletion(
        ...
        val taskStates = fork.workflowState?.taskStates ?: TaskStates()
     ...
     */
    suspend fun getBranches(forkId: IDV7): List<ForkBranchModel> = withConnection(null) { conn ->
     ...
     */
    suspend fun cleanupOldForks(olderThan: Instant): Int = withConnection(null) { conn ->
    ...
    private fun ResultSet.toForkBranchModel() = ForkBranchModel(
        ...
        startedAt = getTimestamp("started_at")?.toInstant()?.toKotlinInstant(),
